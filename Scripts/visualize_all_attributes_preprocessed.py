#!/usr/bin/env python3
"""
Comprehensive Weather Data Visualization Script - All Attributes
Uses Preprocessed Data from New Folder Structure

This script generates visualizations for all 13 preprocessed weather attributes:
...
Uses preprocessed data from: Preprocessed_Data/
Outputs to: Preprocessed_Visualizations/
...
class WeatherDataVisualizerPreprocessed:
    def __init__(self, config_path="Scripts/preprocessing_config.yaml"):
        ...
        self.base_path = Path("Preprocessed_Data")
        self.bundesland_base = self.base_path / "Bundesland_aggregation"
        self.germany_base = self.base_path / "Germany_aggregation"
        ...
        # Create visualizations directory (completely new folder)
        self.viz_dir = Path("Preprocessed_Visualizations")
    ...
    def _get_canonical_name(self, filename):
        ...
        base = base.replace('_aggregated', '').replace('_aggregated', '').replace('.csv', '')
            ...
            try:
                ...
                if datetime_col in df.columns:
                    ...
                    if value_col:
                        ...
                        print(f"  Warning: No value column found in {file_path.name}. Available: {list(df.columns)}")
    ...
    def load_germany_data(self, folder_path, value_columns):
        ...
        for file_path in csv_files:
            ...
            if 'Germany' in file_path.name or 'germany' in file_path.name.lower() or len(csv_files) == 1:
                try:
                    df = pd.read_csv(file_path, low_memory=False)
                    ...
                    if datetime_col in df.columns:
                        ...
                        if value_col:
                            ...
                            print(f"  Warning: No value column found. Available: {list(df.columns)}")
                except Exception as e:
    ...
    def create_monthly_plots(self, attribute_name, bundesland_data, germany_data, 
        ...
        for month in months:
            ...
            ax.set_title(f'{attribute_name.title()} (Preprocessed) - {month_names[month-1]} 2024',
            ...
            plt.tight_layout()
            output_file = monthly_dir / f"{attribute_name}_preprocessed_month_{month:02d}_2024.png"
            plt.savefig(output_file, dpi=self.viz_config['dpi'], bbox_inches='tight')
            plt.close()
            
            print(f"  Created monthly plot: {output_file.name}")
    ...
    def create_full_year_plot(self, attribute_name, bundesland_data, germany_data,
        ...
        ax.set_title(f'{attribute_name.title()} (Preprocessed) - Full Year 2024',
        ...
        plt.tight_layout()
        output_file = output_dir / f"{attribute_name}_preprocessed_full_year_2024.png"
        plt.savefig(output_file, dpi=self.viz_config['dpi'], bbox_inches='tight')
        plt.close()
        
        print(f"  Created full-year plot: {output_file.name}")
    ...
    def create_distribution_plots(self, attribute_name, bundesland_data, germany_data,
        ...
        for month in months:
            ...
            if plot_data:
                ...
                ax.set_ylabel(y_label, fontsize=12)
                ax.set_title(f'{attribute_name.title()} Distribution (Preprocessed) - {month_names[month-1]} 2024',
                           fontsize=14, fontweight='bold', pad=15)
                ax.grid(True, alpha=0.3, axis='y')
                
                plt.tight_layout()
                output_file = dist_dir / f"{attribute_name}_preprocessed_distribution_month_{month:02d}_2024.png"
                plt.savefig(output_file, dpi=self.viz_config['dpi'], bbox_inches='tight')
                plt.close()
                
                print(f"  Created distribution plot: {output_file.name}")
    ...
    def create_statistical_summaries(self, attribute_name, bundesland_data, germany_data,
        ...
        if summary_data:
            ...
            output_file = summary_dir / f"{attribute_name}_preprocessed_monthly_stats_2024.csv"
            summary_df.to_csv(output_file, index=False)
            print(f"  Created summary table: {output_file.name}")
    ...
    def visualize_attribute(self, attribute_name):
        ...
        bundesland_folder = self.bundesland_base / attr_config['bundesland_folder']
        germany_folder = self.germany_base / attr_config['germany_folder']
...
def main():
    ...
    visualizer = WeatherDataVisualizerPreprocessed()
    ...
    print("="*60)
    print("WEATHER DATA VISUALIZATION - ALL ATTRIBUTES (PREPROCESSED)")
    print("="*60)
    print(f"Generating visualizations for {len(attributes)} attributes...")
    print(f"Input data from: Preprocessed_Data/")
    print(f"Output visualizations to: Preprocessed_Visualizations/")
    ...
    print(f"{'='*60}")
    print(f"All visualizations saved to: Preprocessed_Visualizations/")
